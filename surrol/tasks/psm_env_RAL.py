...
from surrol.robots.psm_RAL import Psm1, Psm2
...
class PsmEnv(SurRoLGoalEnv):
    ...
    def _env_setup(self):
        ...
        p.loadURDF(os.path.join(ASSET_DIR_PATH, 'table_RAL/table.urdf'),
                   np.array(self.POSE_TABLE[0]) * self.SCALING,
                   p.getQuaternionFromEuler(self.POSE_TABLE[1]),
                   globalScaling=self.SCALING)

        # for goal plotting
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'sphere_RAL/sphere.urdf'),
                            globalScaling=self.SCALING)
