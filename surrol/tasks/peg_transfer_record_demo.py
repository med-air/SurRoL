...
class PegTransferHaptic(PsmEnv):
    POSE_BOARD = ((0.55, 0, 0.6861), (0, 0, 0))  # 0.675 + 0.011 + 0.001
    WORKSPACE_LIMITS = ((0.50, 0.60), (-0.05, 0.05), (0.686, 0.745))
    SCALING = 5.
    ...
    def __init__(self, render_mode=None, cid = -1):
        super(PegTransferHaptic, self).__init__(render_mode, cid)
        self._view_matrix = p.computeViewMatrixFromYawPitchRoll(
            cameraTargetPosition=(-0.05 * self.SCALING, 0, 0.375 * self.SCALING),
            distance=1.81 * self.SCALING,
            yaw=90,
    ...
    def _env_setup(self):
        super(PegTransferHaptic, self)._env_setup()
        self.has_object = True
        ...
        # peg board
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'peg_board_tao/peg_board.urdf'),
                            np.array(self.POSE_BOARD[0]) * self.SCALING,
                            p.getQuaternionFromEuler(self.POSE_BOARD[1]),
                            globalScaling=self.SCALING)
        self.obj_ids['fixed'].append(obj_id)  # 1
        ...
        for i in self._pegs[6: 6 + num_blocks]:
            pos, orn = get_link_pose(self.obj_ids['fixed'][1], i)
            yaw = (np.random.rand() - 0.5) * np.deg2rad(60)
            obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'block_v1/block.urdf'),
                                np.array(pos) + np.array([0, 0, 0.03]),
                                p.getQuaternionFromEuler((0, 0, yaw)),
                                useFixedBase=False,
                                globalScaling=self.SCALING)
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        if retrived_action[4] == 2:
            ...
            '''Control PSM'''
            self.action[0] = retrived_action[2]/8
            self.action[1] = retrived_action[0]/8
            self.action[2] = retrived_action[1]/8
            self.action[3] = -retrived_action[3]/math.pi*180*0.5/8
...
if __name__ == "__main__":
    env = PegTransferHaptic(render_mode='human')  # create one process and corresponding env
