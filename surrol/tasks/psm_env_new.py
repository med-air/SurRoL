...
# from surrol.robots.psm_simplified import Psm1, Psm2
...
class PsmEnv(SurRoLGoalEnv):
    ...
    #modified limits for match_board, increase y-axis workspace
    WORKSPACE_LIMITS1 = ((0.50, 0.60), (-0.07, 0.07), (0.675, 0.785))
    SCALING = 1.
    ...
    def _env_setup(self):
        ...
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'sphere/sphere.urdf'),
                            globalScaling=0.00000000000000001)
    ...
    def _set_action(self, action: np.ndarray):
        ...
        if self.ACTION_MODE == 'yaw':
            action[3] *= np.deg2rad(15)  # yaw, limit maximum change in rotation
