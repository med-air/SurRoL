...
class PsmEnv(SurRoLGoalEnv):
    ...
    WORKSPACE_LIMITS1 = ((0.50, 0.60), (-0.07, 0.07), (0.675, 0.778))
    SCALING = 1.
    # touch_floor=[0]*100
    def __init__(self,
        ...
        if self.block_gripper or not self.has_object or self._activated < 0:
            ...
            if self._meet_contact_constraint_requirement():
                ...
                if len(contact_List)>0:
                    ...
                    contact_item_pose = get_link_pose(contact_Id, -1)
                    print(f'contacted item id:{contact_Id}, its position:{contact_item_pose}')               
                    #this check is to prevent abnormal result of grsping rings
                    
                    # if contact_item_pose[0][2]<3.62:
                    #     self.touch_floor[contact_Id]=1
                    # if not self.touch_floor[contact_Id]:
                    ...
                    p.changeConstraint(self._contact_constraint, maxForce=2)
        else:
